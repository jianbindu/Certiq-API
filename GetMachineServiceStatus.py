...
for num in range(0, MaxMachineIndex):
    ...
    url = 'https://certiq-api.epiroc.com/v2/machines/' + \
        str(ItemNumber) + '/serviceStatus'
    print('Request service status: '+url)
    response = requests.request("GET", url, headers=headers)
    # 获取返回Json,传入df2暂存.
    machineinfo = response.json()
    df2 = pd.DataFrame(machineinfo, columns=['serviceId', 'serviceAccumulator', 'serviceInterval', 'serviceDescription', 'serviceType', 'serviceNodeIndex',
                                            'servicePredictedDate','hoursLeftToService','serviceStatus'],dtype=str)
    #向df2数据集中加入列ItemNumber
    df2['ItemNumber']=ItemNumber
    #转换df2中alarmTime为datatime格式
    df2['servicePredictedDate'] = pd.to_datetime(
        df2['servicePredictedDate'], infer_datetime_format=True)
    df = df.append(df2, ignore_index=True)
#写入df到Certiq_v2.alarmstatus中
df.to_sql(name='servicestatus',con=engine, index=False, if_exists='append')

#清洗重复数据
servicestatus=pd.read_sql('SELECT * FROM certiq_v2.servicestatus',con=engine)
df=pd.DataFrame(servicestatus)
df.drop_duplicates(inplace=True)
df.to_sql(name='servicestatus',con=engine,index=False,if_exists='replace')
print('GetMachineServiceStatus:Done')